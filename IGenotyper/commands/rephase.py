...
def get_new_bam_file(files,bam,iteration,bam_type):
    new_bam_file = "%s/%s_%s.sam" % (files.tmp,bam_type,iteration)
...
def fix_alignments(files,bam,iteration,bam_type):
    new_sam_file = get_new_bam_file(files,bam,iteration,bam_type)
...
def fix_phased_alignments(files,sample,command_line_tools,iteration):
    bams = {"subreads":files.subreads_to_ref_phased,
            "ccs": files.ccs_to_ref_phased}
    for bam_type in bams:
        fix_alignments(files,bams[bam_type],iteration,bam_type)
