...
from IGenotyper.common.helper import assembly_coords,get_ref_seq,skip_read,get_phased_blocks
...
def msa_coords(files):
    ...
    phased_regions = get_phased_blocks(files,files.phased_blocks)
    phased_regions = get_haps_coords(phased_regions)
    contig_coords = assembly_coords(files.assembly_to_ref_phased)
...
def extract_assembly_sequence(files,chrom,start,end,hap):
    hap_sequence = None
    samfile = pysam.AlignmentFile(files.assembly_to_ref_phased,'rb')
